...
class Trigger:
    ...
    def generate(
        ...
        main.logger.info(
            "mask norm of pair %d-%d: %f" % (source, target, mask_best.abs().sum())
        )
...
class TriggerCombo:
    ...
    def generate(
        ...
        main.logger.info(
            "mask norm of pair %d-%d: %f" % (source, target, mask_best[0].abs().sum())
        )
        main.logger.info(
...
def trigger_fast_train(helper, model, data_iterator, start_epoch, agent_name_key):
    ...
    my_dataloader = DataLoader(
        my_dataset, batch_size=64, shuffle=True, num_workers=8
    )  # create your dataloader

    label_grt = [ele for ele, cnt in Counter(y_train).items() if cnt > size_min]

    indices = []
    # compose a set of images that use to warmup generate universal trigger
    ...
    main.logger.info(
        f"x_extra: {x_extra.shape}, indices: {indices}, y_extra: {y_extra}"
    )
    ...
    if agent_name_key not in local_mat:
        main.logger.info(f"not in dict, local model {agent_name_key}")
        ...
        main.logger.info(f"has been selected before, local model {agent_name_key}")
            ...
            if (WARMUP and step % warmup_steps == 0) or (
                ...
                if WARMUP:
                    ...
                    if np.random.rand() < 0.3:  # scheduler
                        ...
                        main.logger.info(
                            f"random fastest pair: np.random.rand() < 0.3, source: {source}, target: {target}, label_grt_min: {label_grt_min}"
                        )
                    else:
                        diff_sum = mat_diff  # choose the max single direction
                        ...
                        if source == -1:
                            main.logger.info(
                                "source_new == source and no pair is selected"
                            )
                            if step + 1 >= max_steps:
                                break
                            step += 1
                        ...
                        main.logger.info(
                            f"fastest pair: source:{source}, target: {target}, diff_sum[source, target]: {diff_sum[source, target]}, label_grt_min: {label_grt_min}"
                        )
            ...
            if WARMUP:
                ...
                if (
                    ...
                    if length > 0:
                        ...
                        main.logger.info(
                            f"__WARMUP False symmetric, init_mask is None: {init_mask is None}, init_pattern is None: {init_pattern is None}"
                        )
                    ...
                    if (step - max_warmup_steps) % warmup_steps == warmup_steps - 1:
                        ...
                        main.logger.info(
                            f"__WARMUP False symmetric, mask_size_avg: {mask_size_avg}, mat_warm:{mat_warm.shape}"
                        )
                    ...
                    if length > 0:
                        ...
                        choice = np.random.choice(
                            idx_source, length, replace=False
                        )  # choose number of length images from the indices list
